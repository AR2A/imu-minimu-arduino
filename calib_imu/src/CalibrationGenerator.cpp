...
arma::vec CalibrationGenerator::GetMagCentroid(ImuDataset & set){
	...
	for(size_t i=1;i<set.size();i++){
		if(set[i].Magnetometer(0)<min(0)){min(0)=set[i].Magnetometer(0);}
		if(set[i].Magnetometer(1)<min(1)){min(1)=set[i].Magnetometer(1);}
		if(set[i].Magnetometer(2)<min(2)){min(2)=set[i].Magnetometer(2);}
		if(set[i].Magnetometer(0)>max(0)){max(0)=set[i].Magnetometer(0);}
		if(set[i].Magnetometer(1)>max(1)){max(1)=set[i].Magnetometer(1);}
		if(set[i].Magnetometer(2)>max(2)){max(2)=set[i].Magnetometer(2);}
	}
